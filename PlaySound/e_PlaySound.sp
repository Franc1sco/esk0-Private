...
public Action Event_ChatMessage(int client, int args)
{
...
public Action Event_DisableSong(int client, int args)
{
  if(IsValidClient(client))
  {
    char value[12];
...
public Action Event_Song(int client, int args)
{
  if(IsValidClient(client))
  {
    ShowMOTDPanel(client, "Title", "http://fastdl.gamesites.cz/global/motd/songs.html", MOTDPANEL_TYPE_URL);
  }
}
